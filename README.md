...
SkillPilot AI is a full-stack personalized learning platform that maps your path to career readiness using real-time skill gap analysis, adaptive Bayesian quiz modeling, memory decay tracking, and an AI-powered learning copilot.
...
- [Setup & Run](#setup--run)
  ...
  - [Streamlit UI (optional)](#3-streamlit-ui-optional)
...
| **Adaptive Roadmap** | Prerequisite DAG with locked/unlocked phases; AI re-routes path on poor quiz scores |
| **Bayesian Knowledge Tracing (BKT)** | Quiz results update mastery using Bayes' rule — same model used by Khan Academy |
...
| **Multi-Goal Roadmap** | Steiner-tree algorithm finds shared spine for two career goals at once |
| **PDF Export** | One-click full progress report saved to local Downloads folder |
| **Offline Mode** | Works without the backend — full local fallback with localStorage |
...
| LLM | Groq API (`llama3-8b-8192`) with rule-based fallback |
...
- **Python 3.10+**
- **Node.js 18+**
- A **Groq API key** (free at [console.groq.com](https://console.groq.com)) — optional, falls back to rule-based if not set
...
:: Add your Groq API key (edit .env)
...
> **Note:** The frontend works even if the backend is not running — it automatically falls back to local mode using `localStorage`.

---

### 3. Streamlit UI (optional)

The original Streamlit interface is still available as an alternative frontend:
...
|---|---|---|
| `GROQ_API_KEY` | Optional | Enables LLM-powered goal analysis and AI Copilot responses. Falls back to rule-based logic if not set. |
...
|---|---|---|
| Command Center | `/` | Greeting + goal navigator (screen 1), dashboard (screen 2) |
...
If the FastAPI backend is not running, the frontend automatically switches to **local mode**:
...
Go to **Settings → Export Progress Report** and click **Export PDF Report**.

The PDF is generated entirely in the browser and saved to your local **Downloads** folder as:
